...
class TestMakeAllFiles(unittest.TestCase):
    ...
    def test_make_all_files_dir_error(self):
...
class TestMakeFilePath(unittest.TestCase):
    ...
    def test_wrong_file_type_error(self):
        '''Confirm that make_full_path raises FileTypeError for xls file
        name only.'''
        with self.assertRaises(FileTypeError):
...
class TestInsertBasePath(unittest.TestCase):
    ...
    def test_make_dir(self):
        ...
        '''
        test_dir = get_file_path('test folder', base_path=self.base_path)
        self.assertEqual(test_dir, self.files['test_dir'])

    def test_make_relative_dir(self):
        '''Confirm that insert_base_path returns full path for a relative path name.
        '''
        test_dir = get_file_path('..', base_path=self.base_path)
    ...
    def test_already_full_dir_str(self):
        ...
        '''
        test_dir = get_file_path(str(self.files['test_dir']),
                                 base_path=self.base_path)
        self.assertEqual(test_dir, self.files['test_dir'])
    ...
    def test_make_relative_file(self):
        ...
        '''
        relative_path = '..//Utilities//Testing//test folder//test_file.txt'
        # This call ignores the base path
        test_dir = get_file_path(relative_path, base_path=self.base_path)
        self.assertEqual(test_dir, self.files['text_file'])
        test_dir = get_file_path(relative_path)
        self.assertEqual(test_dir, self.files['text_file'])

    @unittest.skip('Generates default base when no base supplied Test Invalid.')
    def test_make_file_error_no_base(self):
        '''Confirm that insert_base_path raises ValueError  for
        directory name / txt file and no base_dir.
        '''
        with self.assertRaises(ValueError):
            get_file_path('test folder//test_file.txt')

    @unittest.skip('filename validity not checked here.')
    def test_make_file_bad_input(self):
        '''Confirm that insert_base_path raises TypeError for number input.
        '''
        with self.assertRaises(TypeError):
            get_file_path(1, base_path=self.base_path)
